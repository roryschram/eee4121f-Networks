...
def start_ping(net):
    ...
    h1.popen("ping -i 0.1 %s > %s/pingPath.txt"%(h2.IP(), args.dir), shell= True)
...
def function(net):
    ...
    h2 = net.get('h2')
    #print("here")
    fetch = h2.popen('curl -o /dev/null -s -w %%{time_total} %s/http/index.html' % h1.IP(), shell=True, stdout=PIPE)
    time = fetch.stdout.read()
    return float(time)
...
def tcp():
    ...
    while True:
        ...
        for i in range(3):
            meas = function(net)
            recordings.append(meas) 
    # TODO: compute average (and standard deviation) of the fetch
   # times.
    average = sum(recordings)/len(recordings)
    stdeviation = numpy.std(recordings)
